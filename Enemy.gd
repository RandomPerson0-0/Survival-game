...
@export var upgrade_speed : float = 100.0
...
func _on_body_shape_entered(body_rid, body, body_shape_index, local_shape_index):
	...
	#also, if the enemy touches the player, it will temporary stop the enemy from moving
...
func _on_timer_timeout():
	#also for every 5 seconds, the enemy speed will increase to make it challenging
	speed = upgrade_speed
